...
Sub Main
	...
	ReDim F(Fnum)
	SetSolverLF(F,Fstart,Fend,Fstep,Fnum,cc)
	' SetSolverM
	...
	Dim h As String,ra As String,ri As String,phi As String,N As String,O As String, cPathStr As String, wPathStr As String, wT As String
		...
		O = "1"
		' wT = "0.25"
		wT = "0.0002546" ' [m], 30 AWG
		h  = 1.1*CStr(2*CDbl(wT)*CDbl(N))
		' MsgBox h
		' cPathStr = "currentWire"
		' wPathStr = "Wire"
		Dim cw2 As String, w2 As String, cw0 As String, w0 As String
		cw0 = "cw0"
		w0 = "w0"
		cPathStr = "cw1"
		wPathStr = "w1"
	...
	' 3) Set vacuum objects around current(path) to increase meshing
		Dim sleeveTH As String, wT2 As String
		sleeveTH = "0.000127" '[m]
		wT2 = "0.0003816"
		'sleeveTH = CStr(1.1*CDbl(wT))
		SetCoilWireCurrent(h,ra,ri,phi,N,O,cPathStr)
		SetCoilWireCurrent(h,ra,ri,phi,N,O,cw2)
		SetCoilWireCurrent(h,ra,ri,phi,N,O,cw0)

		' ThickenWire(cPathStr,wT)
		' ThickenWire(cw2,CStr(CDbl(wT) + CDbl(sleeveTH)))
		ThickenWire(cw0,wT)
		...
		'     .Material "Air"
		'     .Xrange "-"& CStr(cbox*CDbl(xminb)), CStr(cbox*CDbl(xmaxb))
		'     .Yrange "-"& CStr(cbox*CDbl(yminb)), CStr(cbox*CDbl(ymaxb))
		'     .Zrange "-"& CStr(CDbl(zminb)/4), CStr(cbox*CDbl(zmaxb))
		'     .Create
		'End With

		' Solid.Subtract "component1:cw2", "component1:cw0"
		'Solid.Insert "component1:cw2", "component1:cw1"
		'Solid.Intersect "component1:cw2", "component1:cw1"
End Sub
...
Sub SetSolverLF(ByRef  F() As String,  Fstart As Double, Fend As Double, Fstep As Double, Fnum As Integer, cc As Integer)
